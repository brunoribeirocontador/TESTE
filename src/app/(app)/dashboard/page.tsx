...
export default async function DashboardPage() {
  ...
  const [totalEmpresas, totalParcelamentosAtivos, leadsAbertos, tarefasAbertas, tarefasAtrasadas] =
    await Promise.all([
      db.empresa.count(),
      db.parcelamento.count({ where: { status: "ATIVO" } }),
      db.lead.count({ where: { estagio: { notIn: ["GANHO", "PERDIDO"] } } }),
      db.tarefaOnboarding.count({ where: { status: { not: "CONCLUIDA" } } }),
      db.tarefaOnboarding.count({
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Link href="/crm" className="block">
          <Card label="Leads em aberto no funil" value={String(leadsAbertos)} />
        </Link>
        <Link href="/tarefas" className="block">
          <Card label="Tarefas de implantação em aberto" value={String(tarefasAbertas)} />
        </Link>
        <Link href="/tarefas?status=" className="block">
          <Card
            label="Tarefas de implantação atrasadas"
            value={String(tarefasAtrasadas)}
            highlight={tarefasAtrasadas > 0}
          />
        </Link>
      </div>
